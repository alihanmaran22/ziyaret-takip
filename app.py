import streamlit as st
import gspread
import pandas as pd
from oauth2client.service_account import ServiceAccountCredentials

# --- AYARLAR (Aynı) ---
PRIVATE_KEY = """-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----"""

creds_dict = {
  "type": "service_account",
  "project_id": "innate-paratext-499214-r1",
  "private_key": PRIVATE_KEY,
  "client_email": "ziyaret-bot@innate-paratext-499214-r1.iam.gserviceaccount.com",
  "client_id": "117581869089406085398"
}

# --- OTURUM YÖNETİMİ ---
if 'ziyaret_listesi' not in st.session_state:
    st.session_state.ziyaret_listesi = []

# --- ARAYÜZ ---
st.title("💊 Ziyaret Giriş Paneli")

# 1. Girişler
doktor = st.text_input("Doktor Adı:")
kurum = st.text_input("Kurum:")

if st.button("Listeye Ekle"):
    if doktor and kurum:
        st.session_state.ziyaret_listesi.append([doktor, kurum])
    else:
        st.warning("Eksik bilgi var!")

# 2. Liste Görünümü
st.write(f"### 📋 Listede Bekleyenler ({len(st.session_state.ziyaret_listesi)} doktor)")
st.table(st.session_state.ziyaret_listesi)

# 3. Toplu Kaydet
if st.button("Tümünü Excel'e Kaydet"):
    creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive'])
    client = gspread.authorize(creds)
    sheet = client.open("Frekans").sheet1
    
    for row in st.session_state.ziyaret_listesi:
        sheet.append_row(row)
    
    st.session_state.ziyaret_listesi = [] # Listeyi temizle
    st.success("Tümü başarıyla kaydedildi!")
    st.rerun()
